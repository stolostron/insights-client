...
func Test_SetDefaultInt_01(t *testing.T) {
	...
	if property != 1234 {
		t.Errorf("Failed testing setDefaultInt()  Expected: %d  Got: %d", 1234, property)
	}
}
...
func Test_SetDefaultBool_01(t *testing.T) {
	...
	var property bool
	setDefaultBool(&property, "ENV_VARIABLE_NOT_DEFINED", true)

	if property != true {
		t.Errorf("Failed testing setDefaultBool()  Expected: %t  Got: %t", true, property)
	}
}
